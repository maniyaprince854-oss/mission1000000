...
const MIN_ZOOM = 0.1
const MAX_ZOOM = 5.0
const TASKS_PER_RING = 8   // tasks per orbit ring before wrapping
const RING_GAP = 0.72      // multiplier of taskOrbit for each extra ring
...
function Spark({ data, color, w = 72, h = 20 }) {
  if (!data || !data.some(v => v > 0)) return <div style={{ width: w, height: h }} className="opacity-20 bg-[#111] rounded" />
  ...
  const apts = [...pts.split(' '), `${(w - pad).toFixed(1)},${(h - pad).toFixed(1)}`, `${pad},${(h - pad).toFixed(1)}`].join(' ')
  return (
...
export default function Analyse() {
  ...
  const goalOrbit = Math.max(80, minDim * 0.34)
  const taskOrbit = Math.max(44, minDim * 0.17)
  const centerR   = Math.max(28, minDim * 0.063)
  const goalR     = Math.max(22, minDim * 0.05)
  const taskR     = Math.max(11, minDim * 0.024)
  const cx        = size.w / 2
  const cy        = size.h / 2
  const labelSz   = Math.max(7, taskR * 0.72)
  const gLabelSz  = Math.max(8, goalR * 0.36)
  const invZ      = 1 / zoom
  ...
  const taskNodes = goalNodes.flatMap(gn => {
    ...
    return gt.map((task, j) => {
      const ring         = Math.floor(j / TASKS_PER_RING)
      const idxInRing    = j % TASKS_PER_RING
      const countInRing  = Math.min(TASKS_PER_RING, gt.length - ring * TASKS_PER_RING)
      const spread       = countInRing === 1 ? 0 : Math.min(1.35, (countInRing - 1) * 0.36)
      const tAngle       = gn.angle + (spread * (idxInRing - (countInRing - 1) / 2)) / Math.max(1, countInRing - 1)
      const orbit        = taskOrbit + ring * taskOrbit * RING_GAP
      return {
        ...task,
  ...
  const heatVal = useCallback((goalId) => {
    ...
    return Math.min(1, total / 100) // 100 min in 7 days = max heat
  }, [spark7])
  ...
  const onMove = (e) => {
    if (dragRef.current) {
      const pos = evPos(e)
      setDrag(d => ({ ...d, ...pos }))
      setDropTarget(goalNodesRef.current.find(gn => Math.hypot(gn.x - pos.x, gn.y - pos.y) < goalR + 12)?.id || null)
      return
    }
    if (bgPanRef.current) {
        ...
        return (<>
          ...
          <div className="space-y-1.5">
            ...
            {gts.map(t => (
              ...
              >
                {t.isRunning
                  ? <span className="relative w-2.5 h-2.5 flex-shrink-0"><span className="animate-ping absolute inset-0 rounded-full bg-[#10b981] opacity-60"/><span className="absolute inset-0 rounded-full bg-[#10b981]"/></span>
                  : t.status === 'completed'
        ...
        return (<>
          <div className="p-3 bg-[#111] border border-[#1a1a1a] rounded-2xl">
            <div className="flex items-center gap-1.5 mb-2">
              {tg ? <><div className="w-2 h-2 rounded-full" style={{ backgroundColor: tg.color }} /><span className="text-[10px] font-bold uppercase tracking-wide" style={{ color: tg.color }}>{tg.name}</span></>
            ...
            {selTask.isRunning && (
              <div className="flex items-center gap-1.5 mt-2">
                <span className="relative w-2 h-2"><span className="animate-ping absolute inset-0 rounded-full bg-[#10b981] opacity-60"/><span className="absolute inset-0 rounded-full bg-[#10b981]"/></span>
    ...
    <div className="flex flex-col md:flex-row h-full bg-[#0C0C0C] overflow-hidden">
      ...
      <div ref={containerRef} className="flex-1 relative overflow-hidden min-h-0">
        ...
        <div className="absolute top-3 left-3 right-3 z-10 flex items-center gap-2 flex-wrap md:top-3">
          ...
          <div className="flex items-center gap-1.5 bg-[#141414]/90 backdrop-blur-sm border border-[#1E1E1E] rounded-xl px-2.5 py-1.5 flex-1 sm:flex-none sm:w-40 min-w-0">
        ...
        >
          <defs>
            ...
            <radialGradient id="cGlow" cx="50%" cy="50%" r="50%">
              <stop offset="0%" stopColor="#F0C040" stopOpacity="0.08" />
          ...
          <g transform={`translate(${pan.x},${pan.y}) scale(${zoom})`}>
            ...
            {/* Ambient glow + orbit ring */}
            <circle cx={cx} cy={cy} r={goalOrbit * 0.86} fill="url(#cGlow)" />
            <circle cx={cx} cy={cy} r={goalOrbit} fill="none" stroke="#1b1b1b" strokeWidth={invZ} strokeDasharray={`${3 * invZ} ${7 * invZ}`} />

            {/* center → goal (gradient) */}
            {goalNodes.map(gn => (
              <path key={`cl-${gn.id}`} d={curvePath(cx, cy, gn.x, gn.y)}
                stroke={`url(#lg-${gn.id})`} strokeWidth={2 * invZ}
                opacity={hasSearch && !matchGoal(gn) ? 0.03 : 1} fill="none"
              />
            ))}
            ...
            {taskNodes.map(tn => {
              ...
              if (!gn) return null
              return (
                <path key={`tl-${tn.id}`} d={curvePath(gn.x, gn.y, tn.x, tn.y, 0.1)}
                  stroke={tn.goalColor} strokeWidth={invZ}
                  strokeOpacity={hasSearch && !matchTask(tn) ? 0.02 : 0.16} fill="none"
                />
              )
            })}
            ...
            {drag && (() => {
              ...
              if (!sg) return null
              return <line x1={sg.x} y1={sg.y} x2={drag.x} y2={drag.y} stroke="#F0C040" strokeWidth={1.5 * invZ} strokeDasharray={`${5*invZ} ${3*invZ}`} strokeOpacity={0.7} />
            })()}
            ...
            {taskNodes.map(tn => {
              ...
              return (
                <g key={tn.id} transform={`translate(${tx},${ty})`}
                  ...
                  style={{ cursor: isDragging ? 'grabbing' : 'grab', opacity: dimmed ? 0.1 : 1 }}
                >
                  {/* Running: double pulse */}
                  {isRunning && (<>
                  ...
                  {/* Today worked: subtle steady glow */}
                  {workedToday && !isRunning && (
                    <circle r={r + 5} fill="none" stroke="#10b981" strokeWidth={invZ} strokeOpacity={0.22} />
                  )}

                  {/* Selection ring */}
                  {isSel && <circle r={r + 8} fill="none" stroke={tn.goalColor} strokeWidth={1.5 * invZ} strokeOpacity={0.45} />}

                  {/* Body */}
                  <circle r={r}
                    fill={isDone ? '#0d0d0d' : `${tn.goalColor}1a`}
                    stroke={isRunning ? '#10b981' : tn.goalColor}
                    strokeWidth={(isDragging ? 2.5 : isSel ? 2 : 1.5) * invZ}
                    strokeOpacity={isDone ? 0.2 : 0.7}
                  ...
                  {isDone
                    ...
                    : isRunning
                      ...
                      : mins > 0
                        ? <text x={0} y={r * 0.3} textAnchor="middle" fontSize={r * 0.56} fill={tn.goalColor} fontWeight="bold" fontFamily="monospace">{mins}m</text>
                        : <circle r={r * 0.27} fill={tn.goalColor} fillOpacity={0.4} />
                  }

                  {/* Name label */}
                  <text x={0} y={r + labelSz + 4} textAnchor="middle" fontSize={labelSz}
                    fill={isSel ? tn.goalColor : isDone ? '#3a3a3a' : '#5a5a5a'} fontWeight={isSel ? 'bold' : 'normal'}
                    style={{ pointerEvents: 'none' }}
                  >{trunc(tn.title, 16)}</text>
                </g>
              )
            })}
            ...
            {goalNodes.map(gn => {
              ...
              return (
                ...
                >
                  ...
                  {isDone && (
                    <circle r={R + 4} fill="none" stroke="#F0C040" strokeWidth={2 * invZ}>
                      <animate attributeName="stroke-opacity" values="0.6;0.15;0.6" dur="2.5s" repeatCount="indefinite" />
                    </circle>
                  )}

                  {/* Drop glow */}
                  {isDropTgt && <circle r={R + 16} fill={`${gn.color}0d`} stroke={gn.color} strokeWidth={2 * invZ} strokeOpacity={0.8} filter="url(#glow)" />}
                  {/* Selection ring */}
                  {isSel && <circle r={R + 10} fill="none" stroke={gn.color} strokeWidth={1.5 * invZ} strokeOpacity={0.3} strokeDasharray={`${4*invZ} ${4*invZ}`} />}

                  {/* Body */}
                  <circle r={R} fill="#111" stroke={gn.color} strokeWidth={(isSel ? 2.5 : 1.5) * invZ} strokeOpacity={isSel ? 0.9 : 0.55} />
                  {/* Progress arc */}
                  <circle r={inner} fill="none" stroke={gn.color} strokeWidth={sw * invZ}
                    strokeDasharray={circ} strokeDashoffset={off} strokeLinecap="round" strokeOpacity={0.7} transform="rotate(-90)"
                  />

                  {/* % */}
                  <text x={0} y={-R * 0.12} textAnchor="middle" fontSize={Math.max(9, R * 0.44)} fill={isDone ? '#F0C040' : gn.color} fontWeight="900" fontFamily="monospace">
                    {progress.percentage}%
                  </text>
                  <text x={0} y={R * 0.34} textAnchor="middle" fontSize={Math.max(6, R * 0.27)} fill="#555" fontFamily="monospace">
                    {progress.completed}/{progress.total}
                  </text>

                  {/* Done badge */}
                  {isDone && (
                    <text x={R * 0.68} y={-R * 0.68} textAnchor="middle" fontSize={Math.max(8, R * 0.4)}>🏆</text>
                  )}
                  ...
                  {isCollapsed && collCount > 0 && (
                    <g transform={`translate(${R * 0.7},${-R * 0.7})`}>
                      ...
                      <text x={0} y={Math.max(7, R * 0.3) * 0.38} textAnchor="middle" fontSize={Math.max(6, R * 0.24)} fill="#000" fontWeight="900">{collCount}</text>
                    </g>
                  )}

                  {/* Name */}
                  <text x={0} y={R + gLabelSz + 6} textAnchor="middle" fontSize={gLabelSz}
                    fill={isSel ? gn.color : '#888'} fontWeight="bold" style={{ pointerEvents: 'none' }}
                  >{trunc(gn.name, 18)}</text>

                  {isCollapsed && (
                    <text x={0} y={R + gLabelSz * 2 + 9} textAnchor="middle" fontSize={Math.max(5, gLabelSz * 0.75)} fill={gn.color} fillOpacity={0.55}>▲ collapsed</text>
                  )}
                </g>
              )
            })}
            ...
            {/* ── Center ── */}
            <g transform={`translate(${cx},${cy})`} onClick={(e) => { e.stopPropagation(); setSelected(null) }} style={{ cursor: 'pointer' }}>
              <circle r={centerR} fill="#0d0d00" stroke="#F0C040" strokeWidth={2 * invZ} strokeOpacity={0.8} />
              <circle r={centerR - 4} fill="none" stroke="#F0C040" strokeWidth={0.5 * invZ} strokeOpacity={0.15} strokeDasharray={`${3*invZ} ${5*invZ}`} />
              <text x={0} y={-centerR * 0.1} textAnchor="middle" fontSize={Math.max(8, centerR * 0.33)} fill="#F0C040" fontWeight="900" fontFamily="monospace" letterSpacing={1}>MISSION</text>
              <text x={0} y={centerR * 0.3}  textAnchor="middle" fontSize={Math.max(8, centerR * 0.33)} fill="#F0C040" fontWeight="900" fontFamily="monospace" letterSpacing={1}>10000</text>
            </g>

            {/* ── Unassigned cluster ── */}
            {unassigned.length > 0 && (
              <>
                <text x={14} y={size.h - 74} fontSize={7} fill="#222" fontWeight="bold" letterSpacing={1}>UNASSIGNED ({unassigned.length})</text>
                {unassigned.map((t, i) => {
                  const ux  = 20 + (i % 5) * (taskR * 2.5 + 6)
                  const uy  = size.h - 54 + Math.floor(i / 5) * (taskR * 2.5 + labelSz + 8)
                  ...
                  return (
                    ...
                    >
                      ...
                      {t.status === 'completed' ? <circle r={taskR * 0.34} fill="#10b981" /> : <circle r={taskR * 0.25} fill="#222" />}
                      <text x={0} y={taskR + labelSz + 3} textAnchor="middle" fontSize={labelSz} fill="#333" style={{ pointerEvents: 'none' }}>{trunc(t.title, 10)}</text>
                    </g>
                  )
                })}
              </>
      ...
      <div className="hidden md:flex w-72 xl:w-80 border-l border-[#1E1E1E] bg-[#080808] flex-col overflow-hidden flex-shrink-0">
        <div className="px-5 py-4 border-b border-[#1E1E1E] flex items-center justify-between flex-shrink-0">
          ...
          {selected && <button onClick={() => setSelected(null)} className="p-1 text-[#333] hover:text-white transition-colors"><X size={14} /></button>}
        </div>
        ...
        <div className="md:hidden fixed inset-0 z-40 flex flex-col justify-end pointer-events-none">
          ...
          <div className="relative bg-[#0a0a0a] border-t border-[#1E1E1E] rounded-t-2xl max-h-[70vh] flex flex-col pointer-events-auto shadow-2xl">
            <div className="relative flex items-center justify-between px-4 py-3 border-b border-[#1a1a1a] flex-shrink-0">
              ...
              <button onClick={() => setSelected(null)} className="p-1 text-[#333] hover:text-white mt-1.5"><ChevronDown size={18} /></button>
            </div>
